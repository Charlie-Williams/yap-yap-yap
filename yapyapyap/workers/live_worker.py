...
def main():
    ...
    while not stop.is_set():
        ...
        # Normally both streams grow together, so the aligned length is the
        # shorter of the two. In MIC-ONLY mode (macOS with no loopback device)
        # the system stream is an empty placeholder that never grows, so fall
        # back to the mic's length - otherwise min() pins us at 0 and live
        # captions would never fire.
        available = mic_avail if sys_avail <= 0 else min(mic_avail, sys_avail)
